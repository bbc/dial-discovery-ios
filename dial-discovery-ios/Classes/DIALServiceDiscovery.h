...
//------------------------------------------------------------------------------
/**
 * The main class for device discovery using the DIAL protocol. It discovers devices that run a DIAL server and HbbTV applications.
  ...
  *
  *  The list of discovered devices can be obtained by calling the getDiscoveredDIALDevices() method.
  *
...
/**
 *  All currently discovered devices as an array of DIALDevice objects.
 */
@property (nonatomic, readonly, getter=getDevices) NSArray* devices;

/**
 *   All currently discovered devices (and their properties) as a JSON string.
 */
@property (nonatomic, readonly, getter=getDevicesJSON) NSString* devicesJSON;
 ...
 *  Get all devices discovered via the DIAL protocol till now.
 *
 *  @return device descriptions as JSON
 */
